"""Aggregate EU members into one "EU" node and drop intra-EU trade."""
...
def agg_eu(trade_data: pl.dataframe.frame.DataFrame, eu_iso: list):
    """
    ...
    # iso becomes "EU"
    trade_data_eu = trade_data.with_columns(
    ...
    sum_cols = trade_data_eu.select(
        cs.starts_with("net_wgt") | cs.starts_with("primary_value")
    ).columns
    groupby_cols = [_ for _ in trade_data_eu.columns if _ not in sum_cols]

    trade_data_agg_eu = trade_data_eu.group_by(groupby_cols).agg(
...
input_data_agg_eu.write_parquet(snakemake.output[0], compression="gzip")
