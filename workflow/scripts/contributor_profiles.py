"""Compute per-country node-level trade metrics for each network."""
        ...
        rows.append(
            {
                ...
                "primary_value_per_partner_imp": pv_imp / nb_imp
                if nb_imp > 0
                else None,
...
def contributor_profiles(edge_list_dict: dict) -> pl.DataFrame:
    ...
    result = pl.concat(
        [unit_node_metrics(d) for d in edge_lists], how="vertical_relaxed"
    )
...
result = contributor_profiles(edge_list_dict=edge_list_dict)
