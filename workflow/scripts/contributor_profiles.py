...
def unit_contributor_profiles(unit_edge_list_dict: dict,
    ...
    for x in countries:
        ...
        rows.append({
            ...
            "net_wgt_exp": wgt_exp,
            "net_wgt_imp": wgt_imp,
            # Export / import orientation ratios (null when no trade recorded)
            "exp_share": pv_exp / pv_tot if pv_tot > 0 else None,
            "imp_share": pv_imp / pv_tot if pv_tot > 0 else None,
            # Trade intensity per partner (null when no edges in that direction)
    ...
    unit_contributor_profiles = pl.from_dicts(rows)
...
def contributor_profiles(
    ...
    import value, net export and import weight, export/import orientation ratios,
    and trade intensity per partner. The network is directed and weighted.
...
# Compute contributor profiles for each aggregation level
results = [
