"""Deflate UN Comtrade values to constant USD via World Bank unit-value indices."""
...
def reshape_uvi_data(uvi_data: pl.dataframe.frame.DataFrame):
    ...
    uvi_data_exp = (
        uvi_data.select(["economy", "time", "TX.UVI.MRCH.XD.WD"])
        .rename({"TX.UVI.MRCH.XD.WD": "index"})
        .with_columns(flowDesc=pl.lit("Export"))
    )

    uvi_data_imp = (
        uvi_data.select(["economy", "time", "TM.UVI.MRCH.XD.WD"])
        .rename({"TM.UVI.MRCH.XD.WD": "index"})
        .with_columns(flowDesc=pl.lit("Import"))
    )

    # Merge uvi data on export and on import
    uvi_data_reshaped = pl.concat(
        [uvi_data_exp, uvi_data_imp], how="vertical_relaxed"
...
def deflate(
    ...
    data_deflated = uncomtrade_data.join(
        uvi_data_reshaped, on=["reporterISO", "period", "flowDesc"], how="left"
    )

    data_deflated = data_deflated.with_columns(
...
uvi_data = pl.read_csv(snakemake.input[0], separator=";").select(
    ["economy", "time", "TM.UVI.MRCH.XD.WD", "TX.UVI.MRCH.XD.WD"]
)
logging.info(f"\nUVI data:\n {uvi_data}\n")
...
# Save processed data
deflate_uncomtrade.write_parquet(snakemake.output[0], compression="gzip")
