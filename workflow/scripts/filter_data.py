"""Filter merged trade data and aggregate raw flows to FAO product level."""
...
input_data = (
    merged_data.select(snakemake.params["col_keep"])
    ...
    .filter(
        ...
        (
            ~pl.col("reporter_iso").str.contains(
                "|".join(snakemake.params["excluded_iso"])
            )
        ),
        (
            ~pl.col("partner_iso").str.contains(
                "|".join(snakemake.params["excluded_iso"])
            )
        ),
...
thresholds = (
    input_data.group_by("fao_product_agg")
    .agg(
...
input_data = (
    input_data.join(thresholds, on="fao_product_agg", how="left")
    .filter(
        ...
        ((pl.col("net_wgt") > pl.col("min_weight")) | (pl.col("net_wgt").is_null())),
...
input_data = input_data.group_by(groupby_cols).agg(
...
with pl.Config(tbl_rows=-1, tbl_cols=-1):
    for cmd in sorted(input_data.select("fao_code_agg").unique().to_series().to_list()):
        desc = (
            input_data.filter(pl.col("fao_code_agg") == cmd)
...
# Save input data
input_data.write_parquet(snakemake.output[0], compression="gzip")
