"""Download UN Comtrade forest-products data across HS revisions (needs API key)."""
...
def process_param(
    ...
    break_years = sorted(set([_ for _ in HS_versions + [start, stop] if _ >= start]))
    ...
    FAO_HS_codes = pl.read_json(FAO_HS_json).filter(
        pl.col("FAO Code").is_in(fao_divisions)
    )
    codes = [
        cmdCode.to_list()
        for HS in HS_versions_keep
        for cmdCode in FAO_HS_codes.select(cs.contains(str(HS))).unique()
    ]

    return zip(years, codes)


def get_uncomtrade_all(
    apikey: str,
    start: int,
    stop: int,
    HS_versions: list,
    ...
    years, codes = map(
        ...
        zip(
            *[
                list(_)
                for _ in process_param(
                    start, stop, HS_versions, FAO_HS_json, fao_divisions
                )
            ]
        ),
    )
    ...
    uncomtrade_batch = [
        ...
        for years, cmdCode in tqdm(
            process_param(start, stop, HS_versions, FAO_HS_json, fao_divisions),
            total=len(years),
        )
    ]
    ...
    # Concatenate all batch
    uncomtrade_data = pl.concat(
        [df for df in uncomtrade_batch if df.shape != (0, 0)], how="vertical_relaxed"
    )

    # Check of years, commodities, and different flows considered
    check_list = (
        sorted(set(uncomtrade_data["period"].unique()))
        == sorted(set([str(_) for years_ in years for _ in years_])),
        sorted(set(uncomtrade_data["cmdCode"].unique()))
        == sorted(set([str(_) for codes_ in codes for _ in codes_])),
        sorted(set(uncomtrade_data["flowCode"].unique()))
        == sorted(set([str(_) for _ in flowCode])),
    )
...
if all(check_list):
    ...
    UN_Comtrade_data.write_parquet(snakemake.output[0], compression="gzip")
else:
