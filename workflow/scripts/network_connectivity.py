"""Compute degree-distribution connectivity metrics per network."""
...
from scipy.stats import (
    kurtosis,  # pyright: ignore[reportMissingImports]
    skew,  # pyright: ignore[reportMissingImports]
)
...
def network_connectivity(edge_list_dict: dict) -> pl.dataframe.frame.DataFrame:
    """
    Function that returns a polar data frame of the network connectivity metrics
    (total number of edges, mean, variance, skewness and kurtosis of the number
    ...
    network_connectivity = pl.concat(
        [
            unit_network_connectivity(unit_edge_list_dict=unit_edge_list_dict)
...
result = network_connectivity(edge_list_dict=edge_list_dict)
