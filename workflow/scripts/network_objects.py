"""Build mirror flows and per-network edge lists from filtered trade data."""
...
def one_side_flows(
    ...
    flow_code: str,
    flow_type: tuple = ("wgt", "value"),
    trader_role: tuple = ("reporter", "partner"),
):
    """
        ...
        mapping = (
            ...
            dict(
                zip(
                    traders,
                    [
                        _.replace("reporter", "exporter").replace("partner", "importer")
                        for _ in traders
                    ],
                )
            )
            ...
            dict(zip(flows, [f"{_}_exp" for _ in flows]))
        )
    elif flow_code == "M":
        mapping = (
            ...
            dict(
                zip(
                    traders,
                    [
                        _.replace("reporter", "importer").replace("partner", "exporter")
                        for _ in traders
                    ],
                )
            )
            ...
            dict(zip(flows, [f"{_}_imp" for _ in flows]))
        )
    # Extract data and format columns
    extraction_formated = (
        data.filter(pl.col("flow_code") == flow_code).rename(mapping).drop("flow_code")
    )

    return extraction_formated


def compute_mirror_flows(
    data: pl.dataframe.frame.DataFrame, report_suffix: tuple = ("_imp", "_exp")
):
    """
    ...
    join_col = [_ for _ in exp_flow.columns if not any(s in _ for s in report_suffix)]

    mirror_flows = exp_flow.join(imp_flow, on=join_col, how="full", coalesce=True)

    return mirror_flows


def unit_edge_list(
    unit_mirror_flows: pl.dataframe.frame.DataFrame,
    year_col: str = "period",
    ...
    if (
        len(unit_mirror_flows.select(year_col).unique()) > 1
        or len(unit_mirror_flows.select(cmd_col).unique()) > 1
    ):
        raise ValueError("edge_list: data must refer to one year and one product only")
    ...
    wgt_col = [
        _ for _ in unit_mirror_flows.columns if any(s in _ for s in ["_exp", "_imp"])
    ]
    wgt_dicts = unit_mirror_flows.select(wgt_col).to_dicts()

    # Build edge_list
    desc_col = [_ for _ in unit_mirror_flows.columns if country_desc in _]
    edge_list = [(exp, imp) for exp, imp in unit_mirror_flows.select(desc_col).rows()]

    # Add weights to edge_list
    edge_list_wgt = [edge + (wgt,) for edge, wgt in zip(edge_list, wgt_dicts)]
...
def edge_lists(
    ...
    result_list = list(
        map(
            lambda df: unit_edge_list(
                df, year_col=year_col, cmd_col=cmd_col, country_desc=country_desc
            ),
            split_mirror,
        )
    )
...
mirror = compute_mirror_flows(input_data)
edges = edge_lists(input_data)
logging.info(f"\nMirror flows:\n {mirror}\n")

# Save mirror flows and edge list dictionnary
mirror.write_csv(snakemake.output.mirror_flows, separator=";")
with open(snakemake.output.edge_lists, "wb") as f:
    pickle.dump(edges, f)
