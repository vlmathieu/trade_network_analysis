"""Download World Bank unit-value-index series used to deflate values."""
...
wb_list = [
    pl.DataFrame(
        wb.data.DataFrame(
            serie,
            time=range(snakemake.params["year_start"], snakemake.params["year_stop"]),
            columns="series",
        ).reset_index()
    )
...
wb_series_data = pl.concat(wb_list, how="align")
logging.info(f"\nWB series:\n {wb_series_data}\n")

check = sorted(snakemake.params["wb_series"]) == sorted(
    [_ for _ in wb_series_data.columns if _ not in {"economy", "time"}]
)
